...
func CreatePhotosXML() ([]byte, error) {
	timelineCutoffDate := time.Now().AddDate(-1, 0, 0)
	entries, err := GetKindyCacheByType(local.KindyTypePhoto)
	if err != nil {
		return nil, err
	}
	entries = slices.DeleteFunc(entries, func(e local.Kindy) bool {
	...
	lastUpdatedTime := entries[0].PublishedAt
	feed := RssFeed{
		Title:         "gerben.dev photos",
		Link:          "https://gerben.dev/photos/",
		Description:   "The latest photos on gerben.dev.",
		PubDate:       lastUpdatedTime.Format(time.RFC1123Z),
		LastBuildDate: time.Now().Format(time.RFC1123Z),
	}
	...
	for _, entry := range entries {
		...
		item := &RssItem{
			...
			PubDate:     entry.PublishedAt.Format(time.RFC1123Z),
			Category:    entry.Tags,
			Content:     &RssContent{Content: img},
		}
		feed.Items = append(feed.Items, item)
	}
